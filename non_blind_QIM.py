...
def get_change(block):
    '''
    这个函数的目的是为block选择合适的修改距离，但是还不太清楚咋选，需要测试
    :param block:
    :return:
    '''
    return 10
            ...
            if(AC!=None):
                for index in AC:
                    if(bin_data[data_index]=='1'):
                        block[index[0],index[1]]+=get_change(block)
                    else:
                        block[index[0], index[1]] -= get_change(block)
                    data_index+=1
...
def Qim_extract(block_dct,secret_length,ori_cover_dct):
    '''
      提取
      :param block_dct:
      :param secret_length:
    ...
    for h in range(Hn):
        for w in range(Wn):
            block=block_dct[8 * h: 8 * (h + 1), 8 * w: 8 * (w + 1)]
            ori_block=ori_cover_dct[8 * h: 8 * (h + 1), 8 * w: 8 * (w + 1)]
            # AC=find_AC(block,num_ac)
            AC=[(2,2),(4,4)]
            if(AC!=None):
                for index in AC:
                    if(block[index[0],index[1]]>=ori_block[index[0],index[1]]):
                        data=data+"1"
                    else:
                        data=data+"0"
...
if __name__ == '__main__':
    ...
    cv2.imwrite("stego_65_no_blind.png",stego_image)
    stego_spa = read_jpeg("stego_65_no_blind.png")
    stego_dct = block_dct(stego_spa)
    PSNR = psnr(cover_spa, stego_spa)
    recover_data=Qim_extract(stego_dct,len(data),ori_dct)
